...
que conserve la misma división: un bloque de barra general (buscador, Listas y Explorar) y
un bloque de usuario con los mismos accesos del menú, el selector de locale y el cierre de
sesión.
...
- **WHEN** un usuario con sesión abre cualquier página con el Header en un viewport de
  escritorio
- **THEN** la barra general muestra el buscador, el enlace a `/lists` y, si el catálogo
  editorial está habilitado, el enlace a Explorar
...
- **THEN** ve un bloque de barra general con el buscador, el enlace a `/lists` y Explorar, y
  un bloque de usuario con los mismos accesos del menú más el selector de locale y el cierre
  de sesión
