...
La barra general SHALL contener únicamente el acceso al buscador del catálogo y, cuando el
catálogo editorial esté habilitado, el enlace a Explorar. La barra general SHALL NOT
mostrar en su nivel superior enlaces a las superficies personales del usuario
(`/me/diary`, `/me/feed`, `/me/favorites`, `/me/lists`, `/me/collection`).
...
que conserve la misma división: un bloque de barra general (buscador y Explorar) y un
bloque de usuario con los mismos accesos del menú, el selector de locale y el cierre de
sesión.
...
- **WHEN** un usuario con sesión abre cualquier página con el Header en un viewport de
  escritorio
- **THEN** la barra general muestra el buscador y, si el catálogo editorial está
  habilitado, el enlace a Explorar
- **AND** no muestra enlaces de nivel superior a diario, feed, favoritos, listas ni
  colección
...
- **THEN** ve un bloque de barra general con el buscador y Explorar, y un bloque de usuario
  con los mismos accesos del menú más el selector de locale y el cierre de sesión
