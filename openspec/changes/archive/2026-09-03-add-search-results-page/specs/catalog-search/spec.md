...
**Reason**: La búsqueda ahora devuelve una lista de candidatos (artistas y álbumes) que pueden
incluir homónimos. Elegir uno automáticamente y navegar a su perfil es exactamente la
ambigüedad "resuelta al azar" que este cambio elimina.
**Migration**: La navegación al perfil de un artista (o al detalle de un álbum) ocurre cuando la
persona hace clic en un resultado concreto de la lista en `/search`, que enlaza a
`/artist/<id>` o `/release-group/<id>`.

### Requirement: Resultado inexistente
**Reason**: El endpoint dejó de resolver a un único artista, por lo que `ARTIST_NOT_FOUND` ya no
se emite desde la búsqueda. Una búsqueda sin coincidencias pasa a ser una lista vacía con un
estado vacío propio, cubierto por la nueva requirement "Sin coincidencias es una lista vacía, no
un error" y por los estados de la página de resultados.
**Migration**: La interfaz muestra un estado vacío cuando `results` llega vacío; no hay que
mapear ningún código de error para el caso "no encontrado".
