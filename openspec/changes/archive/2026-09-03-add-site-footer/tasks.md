...
- [x] 1.1 Crear `messages/es/footer.json` y `messages/en/footer.json` con: encabezados
...
- [x] 1.2 Crear `messages/es/legal.json` y `messages/en/legal.json` con título y
...
- [x] 1.3 Registrar `footer` y `legal` en `src/i18n/request.ts`.
- [x] 1.4 Crear `src/lib/site-links.ts` como fuente única: `CONTACT_EMAIL` (casilla de
...
- [x] 2.1 Crear `src/components/layout/Footer.tsx` (Server Component) que recibe
  `user?: Pick<AuthUser, "id" | "username" | "displayName"> | null` y usa
  `getTranslations("footer")` / `getTranslations("common")`.
- [x] 2.2 Implementar el grupo de identidad: `<Logo />`, `common.appName`,
  `common.tagline`, frase de misión.
- [x] 2.3 Implementar el grupo "Explorar" como `<nav aria-label>` con `Link` de
  `@/i18n/navigation` a `/` (Inicio), `/search`, `/users` y `/about` ("cómo
  funciona").
- [x] 2.4 Implementar el grupo "Cuenta" con render condicional por `user`: anónimo →
  `/auth/login`, `/auth/register`; con sesión → `/users/<username>`, `/me/diary`,
  `/me/settings`.
- [x] 2.5 Implementar el grupo "Recursos": enlaces a `/about` y ayuda.
- [x] 2.5b Implementar el grupo "Conectar": `mailto:` con `CONTACT_EMAIL` visible como
...
- [x] 2.6 Implementar el bloque de atribución con `border-t border-ink-border`,
...
- [x] 4.1 Crear `src/app/[locale]/about/page.tsx`, `terms/page.tsx`,
  `privacy/page.tsx`, `cookies/page.tsx`, `guidelines/page.tsx` como Server
  Components estáticos que leen del namespace `legal`.
- [x] 4.2 Añadir `generateMetadata` por página (título propio + `common.appName`) y
  `robots: { index: false, follow: false }` mientras el contenido sea placeholder.
- [x] 4.3 Usar un layout de página consistente con el resto (`<main>` centrado,
  `font-display` para el `<h1>`, `font-body` para el cuerpo) e incluir una línea de
  "última actualización: pendiente".

## 5. Documentación

- [x] 5.1 Añadir en `docs/03-data/data-licensing.md` una nota de que la atribución de
  las tres capas se materializa en el `Footer` (bloque de atribución) y enlaza a las
  fuentes oficiales.
- [x] 5.2 Omitido: `docs/05-features/README.md` es un índice de *features de
  producto* (el "qué" por feature); el footer es chrome + cumplimiento legal, no
  encaja ahí. La spec de OpenSpec (`specs/site-footer`, `specs/legal-pages`) es la
  fuente de verdad.

## 6. Pruebas

- [x] 6.1 `src/components/layout/Footer.test.tsx`: renderiza un `contentinfo`;
...
- [x] 6.2 Test de humo de las cinco páginas de políticas: renderizan `<h1>` y cuerpo
  en `es` y en `en`, y su metadata declara `noindex`.
- [x] 6.3 Ajustar `src/app/[locale]/layout.test.tsx` si verifica la estructura del
  layout (ahora incluye `<footer>`).

## 7. Verificación

- [x] 7.1 `pnpm run typecheck`, `pnpm run lint`, `pnpm test`, `pnpm run build` en verde.
- [x] 7.2 Verificación manual (dev server): el footer aparece en home, catálogo,
...
- [x] 7.3 `openspec validate add-site-footer --strict` en verde.
