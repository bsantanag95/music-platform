...
- [x] 1.1 Extraer la lógica de `handleSubmit` de `SearchForm` a una función interna
      reutilizable (`runSearch(normalizedQuery)`) que reciba el valor ya normalizado.
- [x] 1.2 Agregar prop opcional `initialQuery?: string` a `SearchForm`; inicializar el
      estado `query` con ese valor.
- [x] 1.3 Agregar `useEffect` que, si `initialQuery` normalizado no está vacío, invoque
      `runSearch` una única vez al montar (guardar con un ref si ya se ejecutó, para no
      repetir en renders posteriores).
- [x] 1.4 Tests: autoejecuta con `initialQuery` presente; no autoejecuta sin `initialQuery`
      ni con uno vacío/solo espacios; no repite la autoejecución en un re-render.

## 2. Página `/search` — leer `q` de la URL

- [x] 2.1 Tipar `searchParams` como `Promise<{ q?: string }>` en
      `src/app/[locale]/(catalog)/search/page.tsx` (Next.js 15, Server Component async).
- [x] 2.2 Pasar `initialQuery={q}` a `<SearchForm />`.

## 3. Componente `HeaderSearch`

- [x] 3.1 Crear `src/components/layout/HeaderSearch.tsx` (Client Component): input +
      submit, sin label visible pero con `aria-label`/label accesible (reusar copy de
      `catalog.search.*` vía `useTranslations("catalog")`).
- [x] 3.2 En submit: normalizar (trim), si vacío no hacer nada; si no, deshabilitar el
      input y llamar a `searchCatalog` (`@/lib/api/catalog`).
- [x] 3.3 En éxito: `router.push('/artist/' + result.artist.id)` (usar `useRouter` de
      `@/i18n/navigation`, igual que `SearchForm`).
- [x] 3.4 En fallo (`ApiError` con `ARTIST_NOT_FOUND` o cualquier otro error):
      `router.push('/search?q=' + encodeURIComponent(normalizedQuery))`.
- [x] 3.5 Tests: envío con resultado navega a `/artist/<id>`; envío sin resultado navega a
...
- [x] 4.1 En `src/components/layout/Header.tsx`, reemplazar el `<Link href="/search">` de
      "Buscar" por `<HeaderSearch />`, como primer elemento del grupo izquierdo.
- [x] 4.2 Actualizar `Header.test.tsx`: el test "muestra un enlace al buscador" deja de
...
- [x] 5.1 En `src/app/[locale]/page.tsx`, mover `<SearchForm />` dentro del bloque
      `{!user && (...)}` que ya envuelve el CTA de registro/login.
- [x] 5.2 Actualizar `src/app/[locale]/page.test.tsx`: agregar/ajustar caso que confirme
...
- [x] 6.2 Si el link "Buscar" del Header tenía una clave propia en `common.json` que queda
      sin uso, confirmar que no rompe otros consumidores antes de tocarla (dejarla si hay
      duda razonable). — `common.search` sigue en uso por `QuickLinks`; se dejó sin tocar.

## 7. Documentación

- [x] 7.1 Actualizar `docs/05-features/home.md`: nota técnica de que el buscador del hero
      queda acotado a visitantes sin sesión.
- [x] 7.2 Si existe un doc de navegación/header en `docs/05-features/`, documentar el
      nuevo `HeaderSearch`; si no existe, no crear uno nuevo solo para esto (fuera de
      alcance). — No existe tal doc; no se creó uno nuevo.
