...
La aplicación SHALL exponer un perfil público en `/{locale}/artist/{id}` para los locales soportados y SHALL mostrar el nombre del artista, su tipo traducido, su biografía cuando exista y su discografía disponible.

#### Scenario: Artista válido en español
- **WHEN** una persona visita `/es/artist/<id-válido>`
- **THEN** la aplicación muestra el nombre y los datos musicales del artista, y las etiquetas de interfaz aparecen en español

#### Scenario: Artista válido en inglés
- **WHEN** una persona visita `/en/artist/<id-válido>`
- **THEN** la aplicación muestra el mismo contenido musical, y las etiquetas de interfaz aparecen en inglés
...
La aplicación SHALL cargar cada carátula después del render inicial mediante el detalle del `releaseGroup`, SHALL mostrar un estado de carga accesible y SHALL usar un fallback cuando no exista carátula.

#### Scenario: Carátula disponible
- **WHEN** el detalle del álbum devuelve una carátula válida
...
- **WHEN** el detalle del álbum no contiene carátula o la consulta falla
