# album-review Specification

## Purpose

La reseña como entidad propia: la postura crítica del usuario sobre una obra —texto largo,
título opcional, rating asociado, editable, una vigente por (usuario, objetivo)—, distinta
del comentario conversacional. Misma forma de objetivo que `rating`/`comment` (3 FK
nullable + CHECK, no polimórfica). En esta versión la escritura se restringe a álbumes en
la capa de validación, no en el esquema.
