...
Verificación del email de una cuenta local mediante un link de un solo uso, con el estado persistido
en `app_user.email_verified_at`. Funciona en modo soft: registra y avisa (banner + reenvío) sin
bloquear el acceso, y deja un helper centralizado para activar el enforcement duro en el futuro
(ADR 0015).

## Purpose

Confirmar que la persona controla el email declarado en el registro, mejorando la recuperación de
cuenta y dejando el estado disponible para un enforcement posterior, sin depender aún de un proveedor
de email real y sin dejar usuarios trabados.
