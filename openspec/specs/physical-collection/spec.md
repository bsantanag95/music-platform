...
página de álbum, la superficie propia `/me/collection` con filtros y la sección pública en el
perfil filtrada por la matriz de visibilidad.
...
audiencia, y eliminarla por su identificador. Quitar una entrada que no existe o que no
pertenece al usuario SHALL responder `404` sin revelar su existencia.

#### Scenario: Editar una entrada propia
- **WHEN** el dueño cambia el formato o los atributos de una entrada propia
- **THEN** la entrada queda actualizada y la superficie refleja el cambio
...
El sistema SHALL permitir al usuario autenticado ver su colección completa en formato de
lista, en orden cronológico descendente por fecha de alta, con paginación, mostrando por
entrada el álbum, su artista, el formato, los atributos y la nota. El sistema SHALL permitir
filtrar la lista por formato y por atributo.

#### Scenario: Ver la colección propia
- **WHEN** un usuario autenticado abre su página de colección
- **THEN** ve sus entradas de la más reciente a la más antigua, paginadas

#### Scenario: Filtrar por formato
- **WHEN** el usuario filtra su colección por `vinyl`
- **THEN** ve únicamente las entradas cuyo formato es `vinyl`
...
seguimiento) aplicada a la audiencia de cada entrada. Si el visitante no tiene permiso, la
respuesta SHALL ser una lista vacía y SHALL NOT revelar si el usuario tiene colección. Si el
...
- **THEN** recibe únicamente las entradas de audiencia `public`
