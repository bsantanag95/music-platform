...
- **`client.ts`** — el único punto del sistema que le habla a `musicbrainz.org`. Implementa una cola en memoria que fuerza como mínimo 1.1 segundos entre requests (el límite real es 1 req/seg, se deja margen), y exige un `User-Agent` identificable o directamente lanza un error — MusicBrainz penaliza fuerte a los clientes anónimos. Expone `searchArtist`, `searchReleaseGroup`, `searchRecording`, `browseReleasesByRecording`, `getArtist`, `getArtistWithRelations`, `browseReleaseGroupsByArtist`, `browseReleasesByReleaseGroup` (todas las ediciones de un álbum, de a 100, con sellos y formatos — reemplazó al lookup del grupo, que devolvía como máximo 25), `getRelease` (tracklist, créditos de autoría y créditos de personal en la misma request). Las **búsquedas por texto** (`searchArtist`, `searchReleaseGroup`, `searchRecording`) y el browse de apariciones de una grabación (`browseReleasesByRecording`) pasan por una caché TTL en memoria (10 min, máx. 200 entradas): la misma consulta dentro de la ventana se sirve desde memoria sin tocar la cola (los re-renders del servidor de la página de resultados — p. ej. cambiar de idioma — no repiten el round-trip a MusicBrainz), y los requests concurrentes idénticos comparten el mismo promise en vuelo. `browseReleasesByRecording` se cachea como búsqueda porque alimenta la sección contextual de `/search` (dato efímero), no una ingesta. Los `get`/`browse` de ingestas **no** se cachean: al abrir una entidad se quiere la respuesta fresca. Los fallos tampoco se cachean (el siguiente request reintenta). Mismo supuesto de proceso único que la cola.
...
- **`mappers.ts`** — traduce el vocabulario de MusicBrainz al vocabulario propio: `mapArtistType` (`Person`/`Group`/... → `person`/`group`) y `mapReleaseGroupCategory` (`primary-type` + `secondary-types` → `studio`/`single_ep`/`compilation`/`live_other`). `normalizeReleaseDate` (precisión completa para la columna `DATE`) y `yearFromMbDate` (solo el año, para mostrar).
...
- **`cover.ts`** — `findOrResolveCover(rg)` implementa el patrón **cover-only**: resuelve y cachea la carátula de un release-group con un `HEAD` a Cover Art Archive (0 llamadas a MusicBrainz, no requiere ingestar el tracklist). Es la única fuente escribible de la carátula (`release_group.cover_thumb_url`, migración `0003`); si el valor cacheado es `null` se re-resuelve en cada acceso (self-heal, mismo criterio que antes aplicaba `ingest-release`). Lo consumen el endpoint cover-only y `album-detail`.
- **`ingest-release.ts`** — `findOrIngestTracklist(releaseGroupId, releaseGroupMbid)` devuelve la edición **representativa** del álbum (`release.is_representative`, ADR 0019) o, en la primera visita, pide todas las ediciones con el browse paginado (`release-editions.ts`), guarda su resumen y la fecha canónica, elige la representativa con `pickRepresentativeRelease` y la ingiere con `ingestReleaseTracklist`. Esa función ingiere UNA edición —representativa o no (las variantes con pistas adicionales se ingieren bajo demanda desde `album-editions.ts`)—: crea las `recording` y `track`, llama a `ingestCredits` por cada canción con créditos propios y guarda los créditos de personal (`personnel-credits.ts`) de la misma respuesta. Normaliza la fecha de la edición con `normalizeReleaseDate` antes de persistirla: MusicBrainz puede entregar `1985`, `1985-06` o `1985-06-15`, pero la columna `release_date` es `DATE` y solo acepta la forma completa; las fechas parciales se guardan como `null` (ver `docs/03-data/sql-model.md`). **Ya no resuelve carátulas** — eso vive en `cover.ts` (`release_group.cover_thumb_url`); la columna legada `release.cover_thumb_url` no se escribe más.
- **`ingest-recording.ts`** — ingesta mínima de una grabación suelta (openspec `add-recording-album-search` / capacidad `catalog-recording-ingestion`): `findOrIngestRecording(seed)` crea o recupera la `recording` por `mbid` (idempotente, nunca pisa una fila enriquecida) e ingiere sus créditos con `ingestCredits`; `albumsFromMbReleases(...)` agrupa las apariciones de la respuesta de browse por `release_group` (año = release más antiguo del grupo) y persiste esos álbumes como stubs; `localAppearanceAlbums(recordingId)` resuelve apariciones ya cacheadas vía `track → release → release_group`. **Prohibido escribir `release`/`track` desde acá**: `findOrIngestTracklist` devuelve el release local existente tal cual, así que un tracklist parcial congelaría el álbum con datos incompletos — las apariciones de la búsqueda se calculan en vivo desde el browse cacheado.
- **`release-editions.ts` / `edition-variants.ts` / `album-editions.ts`** — resumen de todas las ediciones (browse paginado con tope de 5 páginas, upsert idempotente de ediciones y sellos, sincronización en segundo plano con lock por álbum), detección pura de variantes con pistas adicionales (agrupadas por recuento y formatos, contando una sola vez las capas de un SACD) y la lista de una variante bajo demanda (endpoint `…/editions/[editionId]/extra-tracks`).
- **`personnel-credits.ts` / `personnel-levels.ts`** — créditos de personal (relaciones de artista de MusicBrainz sobre la edición y cada grabación), su reemplazo en transacción, la sincronización de álbumes existentes y la clasificación de lectura en integrantes, invitados, producción y sonido, y arte y otros.
- **`album-detail.ts`** — `getAlbumDetail(releaseGroupId)` es el read-model compartido para el detalle de álbum (además agenda con `after()` la sincronización de ediciones y de créditos de personal pendientes): resuelve el `release_group`, reutiliza `findOrIngestTracklist`, consulta tracks ordenados por `disc_number` y `position`, arma los créditos por canción y resuelve la carátula con `findOrResolveCover`. Devuelve un resultado discriminado (`not_found`, `no_editions`, `ok`) que permite distinguir álbum inexistente de álbum sin ediciones ingeribles. Normaliza el response para que `cover` y `release.coverThumbUrl` coincidan siempre (`release_group.coverThumbUrl ?? release.coverThumbUrl` legado). Lo consumen tanto el Server Component de `/[locale]/album/[id]` como el endpoint REST `GET /api/catalog/release-group/[id]`, evitando duplicar la lógica de consulta y ordenamiento.

## `src/services/cover-art.ts`

Un solo helper de URLs: `coverThumbUrl(releaseGroupMbid)` arma la miniatura de 250px en Cover Art Archive a nivel de **release-group** (no de release, porque CAA guarda el arte por edición y solo responde para la que lo carga). `resolveCoverThumbUrl(releaseGroupMbid)` verifica con un `HEAD` si existe carátula y devuelve la URL o `null`. Nunca construye una URL de resolución completa — es la decisión de licencia documentada en `03-data/data-licensing.md` aplicada directamente en código, no solo como política. Quién la usa: `src/services/catalog/cover.ts` (patrón cover-only).
...
- **`api/catalog/search/route.ts`** — `GET ?q=texto`: delega en `searchCatalog` (resolver de
  candidatos: artistas + álbumes, CERO ingesta de discografía/tracklist/carátula, más el contexto
  opcional `songContext` con los álbumes que contienen la canción detectada — ver
  `docs/04-api/contracts.md`). La ingesta pesada ocurre al abrir un resultado.
...
- **`api/catalog/release-group/[id]/route.ts`** — `GET` sobre un álbum ya conocido: llama a `getAlbumDetail` (el read-model compartido), que resuelve el `release_group`, ingiere el tracklist si hace falta, y devuelve el tracklist con duración de cada canción **y sus créditos** (`feat.`). El read-model ordena por `disc_number` y `position`, y distingue `ALBUM_NOT_FOUND` de `NO_EDITIONS_FOUND`.
- **`api/catalog/release-group/[id]/cover/route.ts`** — `GET` cover-only: devuelve `{ cover }` resolviendo la carátula del release-group con un `HEAD` a Cover Art Archive, **sin ingestar el tracklist** (0 llamadas a MusicBrainz). Es lo que usa `LazyCoverImage` en la grilla del perfil, por lo que cargar carátulas de un artista frío no se serializa detrás de la ingesta de cada álbum.
...
- **`catalog.ts`** — `searchCatalog`, `getArtistById`, `getReleaseGroupDetail`, `getReleaseGroupCover` (cover-only): funciones de alto nivel sobre `client.ts`, lo único que los componentes deberían importar para hablar con la API.
