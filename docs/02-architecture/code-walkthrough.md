...
- **`ingest-release.ts`** — `findOrIngestTracklist(releaseGroupId, releaseGroupMbid)` trae la edición "oficial" de un álbum (o la primera disponible) con su tracklist completo, crea las `recording` y `track` correspondientes, y llama a `ingestCredits` por cada canción que tenga créditos propios. Normaliza la fecha de la edición con `normalizeReleaseDate` antes de persistirla: MusicBrainz puede entregar `1985`, `1985-06` o `1985-06-15`, pero la columna `release_date` es `DATE` y solo acepta la forma completa; las fechas parciales se guardan como `null` (ver `docs/03-data/sql-model.md`). **Ya no resuelve carátulas** — eso vive en `cover.ts` (`release_group.cover_thumb_url`); la columna legada `release.cover_thumb_url` no se escribe más.
