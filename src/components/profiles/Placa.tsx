...
export async function Placa({ profile, authenticated }: PlacaProps) {
  ...
  return (
    <section className="flex w-full max-w-2xl flex-col gap-4 sm:flex-row sm:items-start sm:justify-between sm:gap-6">
      <div className="flex min-w-0 gap-4">
        <span
          aria-hidden="true"
          className={`flex size-16 shrink-0 items-center justify-center rounded-lg border font-display text-2xl ${monogramStyle(
            profile.username,
          )}`}
        >
          {monogramLetter(name)}
        ...
        <div className="flex min-w-0 flex-col gap-2">
          <div className="min-w-0">
            <h1 className="truncate font-display text-2xl text-paper">{name}</h1>
