...
import { COLLECTION_VIEW_MODES, type CollectionViewMode } from "./collection-view-mode";

// Iconos mínimos (currentColor), en la misma familia de trazo fino del sistema.
const ICONS: Record<CollectionViewMode, React.ReactNode> = {
  shelf: (
...
// Conmutador del modo de visualización de la colección: grupo de opciones
...
// angostas. El estado activo es el único ámbar en reposo de esta barra.
export function CollectionModeSwitcher({
  ...
  const onKeyDown = (event: React.KeyboardEvent) => {
    ...
    const i = COLLECTION_VIEW_MODES.indexOf(mode);
    const nextIndex =
      event.key === "ArrowRight"
        ? (i + 1) % COLLECTION_VIEW_MODES.length
        : (i - 1 + COLLECTION_VIEW_MODES.length) % COLLECTION_VIEW_MODES.length;
    const next = COLLECTION_VIEW_MODES[nextIndex]!;
    onChange(next);
    document.getElementById(`collection-mode-${next}`)?.focus();
  };
  ...
  return (
    ...
    >
      {COLLECTION_VIEW_MODES.map((value) => {
        const selected = value === mode;
        return (
          <button
            key={value}
            id={`collection-mode-${value}`}
            type="button"
            role="radio"
