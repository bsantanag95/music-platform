...
import { CAMINO_VIEW_MODES, type CaminoViewMode } from "./camino-view-mode";

// Mismos iconos mínimos (currentColor) que `ArtistJourneyModeSwitcher`/
// `WantToListenModeSwitcher`, misma familia de trazo fino del sistema.
const ICONS: Record<CaminoViewMode, React.ReactNode> = {
  detailed: (
...
// Conmutador del modo de visualización de "Mis Caminos", calcado de
// `ArtistJourneyModeSwitcher`: grupo de opciones excluyentes con
// `role="radiogroup"`, tabindex móvil y navegación por flechas.
export function CaminoModeSwitcher({
  mode,
  onChange,
}: {
  mode: CaminoViewMode;
  onChange: (next: CaminoViewMode) => void;
}) {
  ...
  const onKeyDown = (event: React.KeyboardEvent) => {
    ...
    const i = CAMINO_VIEW_MODES.indexOf(mode);
    const nextIndex =
      event.key === "ArrowRight"
        ? (i + 1) % CAMINO_VIEW_MODES.length
        : (i - 1 + CAMINO_VIEW_MODES.length) % CAMINO_VIEW_MODES.length;
    const next = CAMINO_VIEW_MODES[nextIndex]!;
    onChange(next);
    document.getElementById(`camino-mode-${next}`)?.focus();
  };
  ...
  return (
    ...
    >
      {CAMINO_VIEW_MODES.map((value) => {
        const selected = value === mode;
        return (
          <button
            key={value}
            id={`camino-mode-${value}`}
            type="button"
            role="radio"
