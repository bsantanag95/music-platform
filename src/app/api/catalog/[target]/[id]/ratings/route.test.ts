...
describe("rutas sociales de ratings", () => {
  ...
  it("no acepta user_id en la mutación y usa el usuario de sesión", async () => {
    vi.mocked(auth.requireUser).mockResolvedValue({ id, username: "ana", email: "ana@example.com", displayName: null, passwordHash: null, profileVisibility: "public", bio: null, pronouns: null, location: null, timezone: null, avatarUrl: null, onboardedAt: new Date(), emailVerifiedAt: null, createdAt: new Date() });
    vi.mocked(social.resolveSocialTarget).mockResolvedValue(target);
