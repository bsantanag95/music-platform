...
import { UpdateWantedEntryRequestSchema } from "@/lib/api/schemas";
import { requireUser } from "@/services/auth/authorization";
import { removeWantedEntry, updateWantedEntry } from "@/services/collection/wanted";
...
export const PATCH = withErrorHandling(
  async (request: NextRequest, context: { params: Promise<{ entryId: string }> }) => {
    ...
    const parsed = UpdateWantedEntryRequestSchema.safeParse(body);
    if (!parsed.success) {
      throw new ApiError("VALIDATION_ERROR", 400, "La modificación de la entrada no es válida");
    }
    const user = await requireUser();
    const entry = await updateWantedEntry(entryId, user.id, {
...
export const DELETE = withErrorHandling(
  async (_request: NextRequest, context: { params: Promise<{ entryId: string }> }) => {
    ...
    validId(entryId);
    const user = await requireUser();
    await removeWantedEntry(entryId, user.id);
