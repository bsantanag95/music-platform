...
async function main() {
  ...
  if (!pinkFloyd || !dsotm) {
    throw new Error("Corré primero scripts/smoke-test-ingestion.ts para poblar datos reales.");
  }

  const { GET: artistByIdGET } = await import("../src/app/api/catalog/artist/[id]/route");
  const { GET: releaseGroupGET } = await import("../src/app/api/catalog/release-group/[id]/route");
  const { GET: searchGET } = await import("../src/app/api/catalog/search/route");

  await printResponse(
    "1) GET /api/catalog/artist/[id] (perfil directo, endpoint nuevo)",
    await artistByIdGET(new NextRequest(`http://localhost/api/catalog/artist/${pinkFloyd.id}`), {
  ...
  await printResponse(
    ...
    await releaseGroupGET(new NextRequest(`http://localhost/api/catalog/release-group/${dsotm.id}`), {
  ...
  await printResponse(
    ...
    await artistByIdGET(
      new NextRequest("http://localhost/api/catalog/artist/00000000-0000-0000-0000-000000000000"),
